        ...
        if (v == mBtnSubmit) {
            if (mEditTextUserId.getText().toString().length() == 0) {
                ...
                try {
                    ...
                    //################# Get Plain Value ###################
                    //################# Get Plain Value ###################
                    String strUserId = mEditTextUserId.getText().toString();
                    ...
                    //################# Get MasterKey And Encrypt Account Number ###################
                    //################# Get MasterKey And Encrypt Account Number ###################
                    String strMasterKeyAndMasterKey = GetMasterKeyByUserId.getMasterKeyByUserId(strUserId, GlobalData.getStrRegistrationSecurityKey());
...
//                    if (CheckMasterKeyAndSessionId.checkMasterKeyAndSessionId() == true) {
                                getPin(mStrEncryptUserId, mStrEncryptEmail, mStrEncryptMobileNumber, mStrMasterKey);
